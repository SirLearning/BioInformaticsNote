...
- 用户空间：
	- 工具和应用程序
	- GNU LIBC 系统库 (glibc)：是一个实现标准C库函数的可移植的库
		- 应用程序与glibc连接来访问
			- Linux内核
			- 一些常用函数
		- glibc实现了很多接口，这些接口在头文件中定义
			- 如`stdio.h`头文件定义了
				- 很多标准输入`fopen`和输出`printf`函数
				- 所有给定进程的标准流，如`stdin, stdout, stderr`
		- 构建程序时：
			1. GNU编译器自动将符号解析为glibc
			2. 在运行时，再使用libc共享对象的动态链接对其进行解析
		- 嵌入式系统开发中，使用标准glibc有时候有问题，可使用`-nostdlib`：
			- GCC可以取消自动按标准glibc进行符号解释的设置
			- 开发人员可重写标准glibc中的函数
- 内核空间：所有内核组件共享一个地址空间进行操作，是一个整体的内核，和微核系统不同
	- 系统调用接口
		- 初始化模块
		- 进程调度
		- 内存管理
...
awk：一种样式扫描与处理工具  
- 基于文本的样式扫描和处理：和数据库不同，处理的是文本文件，这些文件没有专门的储存格式  
	- 其他工具：grep、sed、sort、find  
	- awk优点：可以完成grep和sed所能完成的所有工作  
- 可以进行：样式装入、流控制、数学运算符、进程控制语句、内置的变量和函数  
- 是一个简单的工具：命令行即可运行；awk是解释执行的，不用编译，且能与shell脚本结合使用  
- awk程序设计语言：样式扫描和处理语言  
  
awk调用：  
- 命令行  
- `-f`调用awk程序  
- 利用命令解释器调用awk程序：在写有awk程序的文本文件第一行加上`#!/bin/awk -f`，并赋予该文件执行权限
